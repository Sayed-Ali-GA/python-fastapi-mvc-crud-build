...
def get_single_comments_for_tea(comment_id: int, db: Session = Depends(get_db)):
...
def update_tea(comment_id: int, comment: CommentSchema, db: Session = Depends(get_db)):
...
def delete_comment(comment_id: int, db: Session = Depends(get_db)):
    ...
    db.commit() 
    return {"message": f"Tea with ID {comment_id} has been deleted"}